...
const CATEGORIES = [
  ...
  { id: 'upcoming', name: 'Coming Soon', icon: 'time' },
]
...
const MoviesScreen = ({ navigation, route }) => {
  ...
  // Content state (consolidated with useMemo)
  ...
  const fetchInitialData = useCallback(async () => {
    try {
      ...
      setFilteredContent(movies.popular)
    } catch (err) {
  ...
  const fetchFilteredContent = async () => {
    try {
      ...
      switch (filterType) {
        case 'genre':
          const genreRes = await getMoviesByGenre[selectedGenre]()
          content = genreRes.data.results || []
          break
        case 'language':
          const langRes = await getPopularByLanguage.movies[selectedLanguage]()
          content = langRes.data.results || []
          break
        case 'year':
          const yearRes = await getMoviesByYear(selectedYear)
          content = yearRes.data.results || []
          break
        case 'category':
          content = allMovies[selectedCategory] || []
          break
      }
  ...
  const currentFilterOptions = useMemo(() => {
    switch (filterType) {
      case 'genre': return MOVIE_GENRES
  ...
  const currentSectionTitle = useMemo(() => {
    switch (filterType) {
      case 'genre':
        return MOVIE_GENRES.find(g => g.id === selectedGenre)?.name || 'Movies'
      case 'language':
        return `${POPULAR_LANGUAGES.find(l => l.id === selectedLanguage)?.name || ''} Movies`
      case 'year':
        return `${selectedYear} Movies`
      case 'category':
        return CATEGORIES.find(c => c.id === selectedCategory)?.name || 'Movies'
      default:
        return 'Movies'
    }
  }, [filterType, selectedGenre, selectedLanguage, selectedYear, selectedCategory])
      ...
      >
        ...
        {allMovies.featured && (
          <TouchableOpacity
            ...
            activeOpacity={0.95}
            onPress={() => handleItemPress(allMovies.featured)}
          >
            <Image
              source={{ uri: getBackdropUrl(allMovies.featured.backdrop_path) }}
            ...
            >
              <View style={styles.featuredContent}>
                ...
                <Text style={styles.featuredTitle} numberOfLines={2}>
                  {allMovies.featured.title}
                </Text>
                <Text style={styles.featuredOverview} numberOfLines={3}>
                  {allMovies.featured.overview}
                </Text>
              </View>
            </LinearGradient>
          </TouchableOpacity>
        ...
        {todayIso && (
          <>
            <View style={styles.regionalHeader}>
              ...
              <Text style={styles.regionalTitle}>Regional Cinema</Text>
            </View>
            ...
            <CustomFetchSection
              title="🇰🇷 Korean Cinema"
              contentType="movie"
              fetchFn={async (_isoDate, page = 1) => getPopularByLanguage.movies.korean(page)}
              onItemPress={handleItemPress}
              todayIso={todayIso}
            />
            ...
            <CustomFetchSection
              ...
              contentType="movie"
              fetchFn={async (_isoDate, page = 1) => getPopularByLanguage.movies.japanese(page)}
              onItemPress={handleItemPress}
              todayIso={todayIso}
            />
            ...
            <CustomFetchSection
              ...
              contentType="movie"
              fetchFn={async (_isoDate, page = 1) => getPopularByLanguage.movies.spanish(page)}
              onItemPress={handleItemPress}
              todayIso={todayIso}
            />
            ...
            <CustomFetchSection
              title="🇫🇷 French Cinema"
              contentType="movie"
              fetchFn={async (_isoDate, page = 1) => getPopularByLanguage.movies.french(page)}
              onItemPress={handleItemPress}
              todayIso={todayIso}
            />
          </>
